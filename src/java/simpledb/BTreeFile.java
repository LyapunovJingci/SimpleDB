...
//		parent.updateEntry(entry);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		for(int i = 0; i < length; i++){
			...
			BTreeEntry tmpEntry = new BTreeEntry(parentEntry.getKey(),parentEntry.getLeftChild(), parentEntry.getRightChild());
			tmpEntry.setRecordId(parentEntry.getRecordId());

			parentEntry.setLeftChild(nextEntry.getRightChild());
			parentEntry.setRightChild(page.iterator().next().getLeftChild());
			page.insertEntry(parentEntry);

			parent.updateEntry(tmpEntry);
			parentEntry = tmpEntry;
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int length = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		//from right to left
		Iterator<BTreeEntry> tmpIterator = rightSibling.iterator();
		//System.out.println(tmpParent.getRecordId());
		for(int i = 0; i < length; i++){
			//  delete right one, parent = right left = parent
			BTreeEntry nextEntry = tmpIterator.next();
			...
			parentEntry.setRightChild(nextEntry.getLeftChild());
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int length = rightPage.getNumTuples();
		Iterator<Tuple> localIterator = rightPage.iterator();
		for (int i = 0; i < length; i++) {
			if (localIterator.hasNext()) {
				Tuple localTuple = localIterator.next();
				rightPage.deleteTuple(localTuple);
				leftPage.insertTuple(localTuple);
			}
		}

		BTreePageId originalRightSiblingId = rightPage.getRightSiblingId();
		if (originalRightSiblingId == null) {
			leftPage.setRightSiblingId(null);
		} else {
			BTreeLeafPage originalRightSibling = (BTreeLeafPage) getPage(tid, dirtypages, originalRightSiblingId, Permissions.READ_WRITE);
			leftPage.setRightSiblingId(originalRightSiblingId);
			originalRightSibling.setLeftSiblingId(leftPage.getId());
			dirtypages.put(originalRightSiblingId, originalRightSibling);
		}
